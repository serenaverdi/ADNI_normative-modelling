...
# normal distribution?
```{r}
ggdensity(df$total_outlier_score, 
          main = "distribution ",
          xlab = "total outlier count")

shapiro.test(df$total_outlier_score)
...
# - Differences of outliers in each group (using chi2)
```{r paged.print=FALSE}
df.stats <- as.data.frame(sapply(X = df[,grep("_bin", names(df),value = T)], FUN = function(x) chisq.test(x, df$diagnosis)$p.value))
names(df.stats) <- "p_value"
setDT(df.stats, keep.rownames = "ROI")
head(df.stats, 20)

#examples
chisq.test(table(df$L_G.S_cingul.Ant_bin, df$diagnosis))
chisq.test(df$L_G.S_cingul.Ant_bin, df$diagnosis)
```
# 1. Controls vs Dementia chi2
```{r}
df.CD.stats <- df[grep("Controls|Dementia", df$diagnosis), ] 
df.CD.stats <- as.data.frame(sapply(X = df.CD.stats[,grep("_bin", names(df.CD.stats),value = T)], FUN = function(x) chisq.test(x, df.CD.stats$diagnosis)$p.value))
names(df.CD.stats) <- "p_value"
setDT(df.CD.stats, keep.rownames = "ROI")
head(df.CD.stats, 20)
...
# 2. MCI vs Dementia chi2
```{r}
df.MD.stats <- df[grep("MCI|Dementia", df$diagnosis), ] 
df.MD.stats <- as.data.frame(sapply(X = df.MD.stats[,grep("_bin", names(df.MD.stats),value = T)], FUN = function(x) chisq.test(x, df.MD.stats$diagnosis)$p.value))
names(df.MD.stats) <- "p_value"
setDT(df.MD.stats, keep.rownames = "ROI")
head(df.MD.stats, 20)
...
```
# 3. Controls vs MCI chi2
```{r}
df.CM.stats <- df[grep("Controls|MCI", df$diagnosis), ] 
df.CM.stats <- as.data.frame(sapply(X = df.CM.stats[,grep("_bin", names(df.CM.stats),value = T)], FUN = function(x) chisq.test(x, df.CM.stats$diagnosis)$p.value))
names(df.CM.stats) <- "p_value"
setDT(df.CM.stats, keep.rownames = "ROI")
head(df.CM.stats, 20)
